...
async function searchPerson(term = '') {
    if (!term) term = document.getElementById('search-person-input').value.toLowerCase();
...
function showPage(pageId) {
    ...
    if (pageId === 'vehicles' && typeof searchVehicle === 'function') {
        searchVehicle();
    }
}
// ==========================================
...
async function searchVehicle(term = '') {
    // Beispiel-Implementierung: Passe an deine Bedürfnisse an
    const resultsDiv = document.getElementById('vehicle-results');
    if (!resultsDiv) return;
    const snapshot = await db.collection('vehicles')
        .where('plate', '>=', term.toUpperCase())
        .where('plate', '<=', term.toUpperCase() + '\uf8ff')
        .limit(10).get();
    resultsDiv.innerHTML = "";
    snapshot.forEach(doc => {
        ...
        resultsDiv.innerHTML += `
            <div class="card p-4 border-l-4 border-blue-600">
                <h4 class="font-bold">${v.plate} - ${v.model}</h4>
            </div>`;
    });
}
// ==========================================
...
function startWantedListener() {
    ...
    .onSnapshot(snapshot => {
        ...
        snapshot.forEach(doc => {
            const p = doc.data();
            list.innerHTML += `<tr><td class="text-red-500 font-bold">${p.firstname} ${p.lastname}</td><td>WANTED</td><td class="text-right"><button class="text-xs text-blue-500" onclick="showPage('persons'); searchPerson('${p.lastname}')">Details</button></td></tr>`;
        });
    });
}
// ==========================================
...
async function loadReports() {
    ...
    snapshot.forEach(doc => {
        const r = doc.data();
        container.innerHTML += `<div class="card p-4">Bericht ${doc.id}: ${r.content} (von ${r.author})</div>`;
    });
}
...
async function renderEmployeePanel() {
    ...
    snapshot.forEach(doc => {
        const u = doc.data();
        container.innerHTML += `<div class="card p-4">Beamter: ${doc.id} (${u.rank}, ${u.department})</div>`;
    });
}
// PLATZHALTER FÜR STRAFRECHNER
function calculatePenalty() {
    const input = document.getElementById('penalty-input').value;
    const resultDiv = document.getElementById('penalty-result');
    // Beispiel-Logik: Ersetze durch echte Berechnung
    resultDiv.innerText = `Vorgeschlagene Strafe für "${input}": 5 Jahre Haft und $10.000 Buße (Platzhalter)`;
}
